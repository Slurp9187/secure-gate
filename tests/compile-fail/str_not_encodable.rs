...
fn main() {
    // The trait is in scope and works for a byte-shaped source...
    let _ = [0u8; 4].to_hex();
    // ...so this failure is about `str` not being an encoding input, not about a
    // missing import. Keeping the import used also stops `-D warnings` from adding
    // an `unused_imports` line to the snapshot on this branch's 1.70 toolchain.
    let _ = "text".to_hex();
}
