...
use secure_gate::{Dynamic, RevealSecret, RevealSecretMut, SecretLen, dynamic_newtype};
...
fn exact_wide(elems: usize) -> Wide {
    ...
    v.extend(std::iter::repeat_n(WIDE_PAYLOAD, elems));
    assert_eq!(
        v.capacity(),
        elems,
...
fn check_box_header_is_metadata_not_payload(size: usize) {
    let pw = exact_pw(size);
    let header = std::ptr::from_ref(pw.expose_secret()).cast::<u8>();
...
fn check_into_wrapper_drops_the_label_not_the_protection(size: usize) {
    ...
    let header = std::ptr::from_ref(shared.expose_secret()).cast::<u8>();
    ...
    assert_eq!(
        std::ptr::from_ref(base.expose_secret()).cast::<u8>(),
        header,
...
fn newtype_heap_lifecycle_traced() {
    ...
    {
        let hex: String = std::iter::repeat_n("d7", 1168).collect();
