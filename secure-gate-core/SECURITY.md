...
Last updated: March 2026 (for v0.9.0-rc.6)
...
| `rand`              | `from_random()` uses system `OsRng` (`rand` 0.9; unit struct, not seedable) and panics on failure; `from_rng()` accepts caller-supplied `TryRngCore + TryCryptoRng` and returns `Result` (e.g. `StdRng` for deterministic tests)                                                         | Use trusted entropy sources; prefer `from_rng()` where RNG failure should be handled explicitly                                                                                                                         |
...
Just as `with_secret` / `with_secret_mut` are the recommended scoped methods for _accessing_
secrets — keeping the exposed reference tightly bound to the closure lifetime —
`Fixed::new_with` is the recommended constructor for _building_ `Fixed` secrets when
