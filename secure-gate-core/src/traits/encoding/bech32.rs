...
//! - **Zeroizing variants**: Prefer `try_to_bech32_zeroizing`, which returns [`EncodedSecret`]
...
pub const fn bech32_code_length(hrp_len: usize, payload_bytes: usize) -> usize {
    ...
    let payload_chars = (payload_bytes / 5)
        .saturating_mul(8)
        // `usize::div_ceil` is stable only from 1.73, above this branch's MSRV of
        // 1.70, so the ceiling division is written out. `payload_bytes % 5` is at
        // most 4, so this operand is at most 32 and the `+ 4` cannot overflow.
        .saturating_add(((payload_bytes % 5) * 8 + 4) / 5);
    hrp_len
        .saturating_add(1)
...
mod tests {
    ...
    fn test_bit_conversion_large_uncapped() {
        ...
        let bits = large_data.len() * 8;
        // `div_ceil` is 1.73+; this branch is 1.70.
        assert_eq!(fes.len(), bits / 5 + usize::from(bits % 5 != 0));
