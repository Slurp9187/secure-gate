...
fuzz_target!(|data: &[u8]| {
    ...
    // 3. SecurePassword – immutable str (never mutable!)
    let pw: SecurePassword = s.as_ref().into();
    let _ = pw.expose_secret(); // &str
                                // NO finish_mut() here – str is immutable, no capacity to shrink

    // 4. SecurePasswordBuilder – the ONLY mutable string type
    let mut builder = SecurePasswordBuilder::from(s.to_string());

    // Use expose_secret_mut() → &mut String, NOT expose_mut()
    ...
    inner.shrink_to_fit(); // manual shrink – finish_mut() not available on this type in 0.3.4

    let _final_pw = builder.into_password();

    // 5. Fixed-size keys – stack + heap paths
    ...
    let _inner: Vec<u8> = *cloneable.into_inner(); // unbox Box<Vec<u8>>
});
