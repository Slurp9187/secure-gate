...
//!
//! This module defines the [`ExportableType`] trait for opt-in serialization.
//! **No library implementations**—users must explicitly implement this for any type they wish to serialize.
//!
//! ## Security Warning
//!
//! **Implementing this trait allows export of secret data, which can lead to
//! permanent exposure if not handled carefully. Only implement for types where
//! export is absolutely necessary and under controlled conditions.**
//!
...
//!
//! **Security Note**: Grep for `impl ExportableType` during code reviews – each
...
//!
//! impl ExportableType for MySecret {}
//! # }
//! ```
pub trait ExportableType: serde::Serialize {
