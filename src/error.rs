...
//! - **No derive macros.** [`Display`](core::fmt::Display) and `std::error::Error`
//!   are written out by hand below. The messages are fixed strings and two `usize`
...
//! [`Display`](core::fmt::Display) is implemented for every build and the `Error`
//! impls are provided behind the `std` feature instead. This is the sole
//! intentional divergence from 0.9 in this module; the variants, messages, and
//! `#[non_exhaustive]` markers are identical.
//!
