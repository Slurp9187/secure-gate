...
- **`ct_eq_auto` signature simplified** (`src/traits/constant_time_eq_ext.rs`) — the `threshold_bytes: Option<usize>` parameter has been removed. `ct_eq_auto` now takes no threshold argument and uses [`CT_EQ_AUTO_THRESHOLD`] (32 bytes) internally, matching the `deserialize` / `deserialize_with_limit` split in `Dynamic`. A new `ct_eq_auto_with_threshold(&self, other, threshold_bytes: usize)` method provides the per-call override for power users. Migration: replace `ct_eq_auto(&b, None)` with `ct_eq_auto(&b)`; replace `ct_eq_auto(&b, Some(n))` with `ct_eq_auto_with_threshold(&b, n)`.
...
- **`CT_EQ_AUTO_THRESHOLD` constant** (`src/traits/constant_time_eq_ext.rs`, `src/lib.rs`) — public `const CT_EQ_AUTO_THRESHOLD: usize = 32` exported from the crate root under `#[cfg(feature = "ct-eq-hash")]`. Documents the default crossover used by `ct_eq_auto` and serves as a readable base for custom multiples in `ct_eq_auto_with_threshold` calls (e.g. `ct_eq_auto_with_threshold(&b, CT_EQ_AUTO_THRESHOLD * 2)`) or threshold boundary assertions in tests. Mirrors the `MAX_DESERIALIZE_BYTES` pattern in `Dynamic`.
- **`ct_eq_auto_with_threshold` trait method** (`src/traits/constant_time_eq_ext.rs`) — power-user companion to `ct_eq_auto`. Takes an explicit `usize` threshold (no `Option`), applies a `.min(4096)` cap to prevent pathological values such as `usize::MAX` from silently forcing `ct_eq` on arbitrarily large inputs, and otherwise mirrors the existing dispatch logic. Default impl provided; required methods remain `len` and `ct_eq_hash` only.
...
- **New `ct_eq_auto` tests** (`tests/ct_eq_suite/auto.rs`) — `threshold_constant_value` (asserts `CT_EQ_AUTO_THRESHOLD == 32` and crate-root re-export), `ct_eq_auto_equals_with_threshold_default` (verifies `ct_eq_auto()` is identical to `ct_eq_auto_with_threshold(CT_EQ_AUTO_THRESHOLD)` for inputs below, at, and above the threshold), and `pathological_threshold_cap` (verifies `usize::MAX` and `4097` behave identically to `4096` on 5000-byte equal and unequal inputs, exercising the `.min(4096)` cap).
- **Proptest threshold coverage extended** (`tests/proptest_suite/ct_eq.rs`) — `ct_eq_auto_threshold_switch` now generates thresholds from five regions via `prop_oneof!`: normal range (10..60), `0` (force hash), `CT_EQ_AUTO_THRESHOLD` (the constant itself), `4090..4100` (cap boundary), and `usize::MAX` (pathological). Expected routing logic updated to account for the `.min(4096)` cap.
...
- **`rand` feature no longer forces `alloc`** (`Cargo.toml`) — `rand?/alloc`, `dep:once_cell`, and `once_cell?/alloc` have been removed from the `rand` feature. `Fixed::from_random()` only uses `OsRng::try_fill_bytes` on a stack array and requires no heap allocation; `rand` now works in pure `no_std`/`no_alloc` builds for `Fixed<T>`. `once_cell` (needed for the per-process keyed BLAKE3 key) has been moved to `ct-eq-hash` where it is actually used. `Dynamic::from_random()` continues to work when `alloc` is also active, since `Dynamic<T>` already requires `alloc` independently. Note: the `ct-eq-hash + rand` keyed mode still requires `std` via `once_cell::sync::Lazy`; use `ct-eq-hash` without `rand` for unkeyed BLAKE3 in `no_std + alloc` environments.
...
- **`static` secrets + `panic = "abort"` footguns documented** (`SECURITY.md`) — `Fixed::new` is `const fn`, so `static SECRET: Fixed<...> = Fixed::new([...])` compiles silently but is never zeroized (Rust does not invoke `Drop` on program-scope statics). Additionally, `panic = "abort"` builds skip all `Drop` impls on panic, meaning secrets in scope at the time of a panic are not cleared. Both limitations are shared by the broader `zeroize` / `secrecy` ecosystem; they are now documented under *Wrappers — Potential weaknesses* with concrete mitigation notes.
- **MSRV CI job added** (`.github/workflows/ci.yml`) — a dedicated `msrv` job now runs `cargo +1.75 check` (default features) on every push and PR, providing authoritative enforcement of the declared `rust-version = "1.75"`. Previously, MSRV was guarded only by Clippy's `--version-lint` flag in the `stable` job. `--features=full` is intentionally excluded from the MSRV job: the `ct-eq-hash` feature transitively depends on `constant_time_eq v0.4.2` (edition2024, requires Rust 1.85+). Full-feature coverage is provided by the `test` matrix running on `stable`.
- **Weak-dependency feature syntax throughout `Cargo.toml`** — every feature entry of the form `pkg/feature` where `pkg` is an optional dependency declared with `dep:` syntax has been changed to `pkg?/feature`. Without `?`, Cargo rejects the activation on MSRV toolchains because the optional dep has no implicit feature name. Affected entries: `rand?/alloc`, `once_cell?/alloc`, `blake3?/zeroize`, `hex?/alloc`, `base64?/alloc`, `bech32?/alloc`, `serde?/alloc`. The non-optional `zeroize/alloc` is unaffected.

### Changed

- **`ct_eq_auto` simplified to zero-argument call** (`src/traits/constant_time_eq_ext.rs`, `CT_EQ_AUTO.md`, `README.md`, `SECURITY.md`) — `ct_eq_auto` no longer accepts a threshold argument. It delegates to `ct_eq_auto_with_threshold(CT_EQ_AUTO_THRESHOLD)` internally, eliminating the awkward `None` / `Some(n)` distinction at every call site. `ct_eq_auto_with_threshold` caps its argument at 4096 via `.min(4096)` — idiomatic for `usize` since there is no meaningful lower bound to enforce. All benchmarks, tests, fuzz targets, and documentation updated.
- **`ct_eq` / `ct_eq_auto` / `ct_eq_hash` guidance rebalanced** (`CT_EQ_AUTO.md`, `README.md`, `SECURITY.md`) — documentation previously over-recommended `ct_eq_auto` as the universal default. Now leads with `ct_eq` for the common case (fixed-size keys, nonces, ≤ 64 bytes) and positions `ct_eq_auto` for variable/large data. `CT_EQ_AUTO.md` overview sentence, Customization "When to use" bullet, and Recommendation section rewritten. `README.md` Equality section expanded to introduce all three methods (`ct_eq`, `ct_eq_hash`, `ct_eq_auto`) side-by-side with examples and a back-reference in `CT_EQ_AUTO.md`. `SECURITY.md` timing-safety table row, Best Practices equality bullet, `ct-eq-hash` feature row, and probabilistic equality note updated to reflect all three methods and their trade-offs.
- **Encoding exposure model documented** (`README.md`, `SECURITY.md`, `src/traits/encoding/`) — `README.md` Encoding section rewritten to match the Equality section's upfront bullet style: three access patterns (direct method, `with_secret` closure, `expose_secret` escape hatch) with security trade-offs and audit-greppability callouts; full method reference table with fallible column and `_expect_hrp` preference for Bech32; consolidated audit grep command; decode-side wrap-immediately note. Stale/incorrect "must call `expose_secret` first" claim removed from `base64_url.rs` module doc. "Audit visibility" security note added to `hex.rs`, `bech32.rs`, and `bech32m.rs`. `SECURITY.md` Encoding/Decoding mitigations block updated with exposure contract and audit grep caveat.
...
```rust
// ct_eq_auto call-site migration
// Before (0.8.0-alpha.1 / earlier rc.1)
secret_a.ct_eq_auto(&secret_b, None);        // → ct_eq_auto(&secret_b)
secret_a.ct_eq_auto(&secret_b, Some(64));    // → ct_eq_auto_with_threshold(&secret_b, 64)
```
