...
- **BREAKING: `dynamic_newtype!(pub Name, generic Vec<u8>)` and `generic String` are now a
  ...
  **Migration.** Delete the word `generic`. No in-tree caller used either form. Non-byte,
  ...
  Mirrors the same change on `main`, re-derived against this branch rather than copied: the
  macro arms and the fixture are identical because `src/macros/` is, but the snapshot is
  blessed on this line's pinned 1.70, and the prose additions avoid the em-dash characters
  that this file's `dynamic_newtype.rs` already carries as mojibake in three places.
...
- **BREAKING: `Fixed::new` now requires `FixedStorage` on the inner type, which makes
  ...
  **The trait's docs name the one configuration that is easy to get wrong.** The claim that
  `Fixed<Vec<u8>>` is dominated by `Dynamic<Vec<u8>>` "in every feature configuration" is not
  quite true. Disable this crate's `alloc` while `zeroize`'s own `alloc` is enabled elsewhere in
  the graph, and `Vec<u8>: Zeroize` holds, so `Fixed<Vec<u8>>` is nameable, while
  `secure_gate::Dynamic` does not exist at all — `error[E0433]`, "the item is gated behind the
  `alloc` feature". Measured on the commit before the bound landed. The configuration is
  incoherent rather than useful, since it tells this crate there is no allocator while using
  one, and the remedy is one line of `Cargo.toml`; but the error gave no hint of it. `main`
  attaches that guidance to the error itself with `#[diagnostic::on_unimplemented]`, which needs
  Rust 1.78 and therefore cannot be used here. On this line the same three points — reach for
...
- **Two suites now trace a newtype's whole lifecycle rather than testing its methods one at
  ...
  with `derive: [WrapperAccess]`. On this line both files avoid three library APIs newer than
  MSRV 1.70 — `iter::repeat_n`, `ptr::from_ref` and, for clippy's `manual_str_repeat`,
  `str::repeat` — and each file's header says so, so a later reader does not replace them.
