...
- **MSRV CI job added** (`.github/workflows/ci.yml`) — a dedicated `msrv` job now runs `cargo +1.75 check` against default and full features on every push and PR, providing authoritative enforcement of the declared `rust-version = "1.75"`. Previously, MSRV was guarded only by Clippy's `--version-lint` flag in the `stable` job.
