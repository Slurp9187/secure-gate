...
- **`rust-version = "1.70"` in `Cargo.toml`** — documents the crate's MSRV. Rust 1.70 (June 2023) covers all dependencies and provides two-plus years of toolchain coverage.
...
- **MSRV CI job added** (`.github/workflows/ci.yml`) — a dedicated `msrv` job now runs `cargo +1.70 check` against default and full features on every push and PR, providing authoritative enforcement of the declared `rust-version = "1.70"`. Previously, MSRV was guarded only by Clippy's `--version-lint` flag in the `stable` job.
- **`blake3/zeroize` weak-dependency syntax** (`Cargo.toml`) — `ct-eq-hash = [..., "blake3/zeroize"]` was changed to `"blake3?/zeroize"`. Without the `?`, Cargo 1.70 rejects the feature activation because `blake3` uses `dep:` syntax and has no implicit feature named `blake3`; the weak form (`?`) is unambiguous and semantically identical since `dep:blake3` is activated in the same feature.
